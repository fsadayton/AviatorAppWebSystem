...
{
    [TestClass]
    public class PersonalCompensationControllerTest
    {
        [TestMethod]
        public void GetIndexTest()
        {
            var controller = new PersonalCompensationController();

            using (ShimsContext.Create())
            {
                var result = controller.Index() as ViewResult;
                Assert.AreEqual("CompensationProviderList",result.ViewName);
            }
        }
